...
def make_folder():
    return ssh_checkout(f'{data["ip"]}', data["user"], data["pswd"],
                        ...
                        f'{data["folder_ext3"]} {data["folder_bad"]} {data["stat_file_dir"]}',
                        "")
...
def create_bad_archive():
    ssh_checkout(data["ip"], data["user"], data["pswd"], f'cd {data["folder_in"]}; 7z a -t{data["type"]} '
                                                         f'{data["folder_out"]}/arx2', "Everything is Ok")
    ssh_checkout(data["ip"], data["user"], data["pswd"], f'cp {data["folder_out"]}/arx2.{data["type"]} '
                                                         f'{data["folder_bad"]}', '')
    ssh_checkout(data["ip"], data["user"], data["pswd"], f'truncate -s 1 '
                                                         f'{data["folder_bad"]}/arx2.{data["type"]}', '')
